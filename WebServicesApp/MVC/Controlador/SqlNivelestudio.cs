...
{
    public static class SqlNivelestudio
    {
        private static DataTable CRUD(string trx,string UidUserLogin, ClassNivelestudio VALUE, out string resultado, string EstadoExistencia = "A")
        {
            resultado = "";
			...
			ParamIn.Add(new ParametersProcedure(nameof(VALUE.nivelestudio), VALUE.nivelestudio, "".GetType().ToString(), OrentationType.In));
            ...
            #endregion
            var table = SqlProcedureExec.ExecProcedure("[dbo].[sp_nivelestudio]", ParamIn, out ParamOut);
        ...
        public static List<ClassNivelestudio> Select(string UidUserLogin,ClassNivelestudio VALUE, out string resultado, string EstadoExistencia)
        {
            resultado = "";          
            string trx = "011048";
           var table =  CRUD(trx, UidUserLogin,VALUE, out resultado);
            List<ClassNivelestudio> retorno = new List<ClassNivelestudio>();
           var retornoAux = (from x in table.AsEnumerable()
                       select new 
                       {
					   
			uidnivelestudio = x.Field<string>("uidnivelestudio"),
			nivelestudio = x.Field<string>("nivelestudio"),
           ...
           ClassNivelestudio cl;
            foreach (var x in retornoAux)
            {

                cl = new ClassNivelestudio();

				 
			  cl.uidnivelestudio = x.uidnivelestudio;
			  cl.nivelestudio = x.nivelestudio;
				cl.aux1 = x.aux1;
                cl.aux2 = x.aux2;
                cl.aux3 = x.aux3;
                cl.aux4 = x.aux4;
