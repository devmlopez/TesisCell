...
{
    public static class SqlTipoidentificacion
    {
        private static DataTable CRUD(string trx,string UidUserLogin, ClassTipoidentificacion VALUE, out string resultado, string EstadoExistencia = "A")
        {
            resultado = "";
			...
			ParamIn.Add(new ParametersProcedure(nameof(VALUE.tipoidentificacion), VALUE.tipoidentificacion, "".GetType().ToString(), OrentationType.In));
            ...
            #endregion
            var table = SqlProcedureExec.ExecProcedure("[dbo].[sp_tipoidentificacion]", ParamIn, out ParamOut);
        ...
        public static List<ClassTipoidentificacion> Select(string UidUserLogin,ClassTipoidentificacion VALUE, out string resultado, string EstadoExistencia)
        {
            resultado = "";          
            string trx = "011032";
           var table =  CRUD(trx, UidUserLogin,VALUE, out resultado);
            List<ClassTipoidentificacion> retorno = new List<ClassTipoidentificacion>();
           var retornoAux = (from x in table.AsEnumerable()
                       select new 
                       {
					   
			uidtipoidentificacion = x.Field<string>("uidtipoidentificacion"),
			tipoidentificacion = x.Field<string>("tipoidentificacion"),
           ...
           ClassTipoidentificacion cl;
            foreach (var x in retornoAux)
            {

                cl = new ClassTipoidentificacion();

				 
			  cl.uidtipoidentificacion = x.uidtipoidentificacion;
			  cl.tipoidentificacion = x.tipoidentificacion;
				cl.aux1 = x.aux1;
                cl.aux2 = x.aux2;
                cl.aux3 = x.aux3;
                cl.aux4 = x.aux4;
