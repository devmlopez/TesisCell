...
{
    public static class SqlTiposerviciotecnico
    {
        private static DataTable CRUD(string trx,string UidUserLogin, ClassTiposerviciotecnico VALUE, out string resultado, string EstadoExistencia = "A")
        {
            resultado = "";
			...
			ParamIn.Add(new ParametersProcedure(nameof(VALUE.uidtiposervicio), VALUE.uidtiposervicio, "".GetType().ToString(), OrentationType.In));
			ParamIn.Add(new ParametersProcedure(nameof(VALUE.tiposervicio), VALUE.tiposervicio, "".GetType().ToString(), OrentationType.In));
            ...
            #endregion
            var table = SqlProcedureExec.ExecProcedure("[dbo].[sp_tiposerviciotecnico]", ParamIn, out ParamOut);
        ...
        public static List<ClassTiposerviciotecnico> Select(string UidUserLogin,ClassTiposerviciotecnico VALUE, out string resultado, string EstadoExistencia)
        {
            resultado = "";          
            string trx = "01600";
           var table =  CRUD(trx, UidUserLogin,VALUE, out resultado);
            List<ClassTiposerviciotecnico> retorno = new List<ClassTiposerviciotecnico>();
           var retornoAux = (from x in table.AsEnumerable()
                       select new 
                       {
					   
			uidtiposervicio = x.Field<string>("uidtiposervicio"),
			tiposervicio = x.Field<string>("tiposervicio"),
           ...
           ClassTiposerviciotecnico cl;
            foreach (var x in retornoAux)
            {

                cl = new ClassTiposerviciotecnico();

				 
			  cl.uidtiposervicio = x.uidtiposervicio;
			  cl.tiposervicio = x.tiposervicio;
			  cl.orden = x.orden;
				cl.aux1 = x.aux1;
                cl.aux2 = x.aux2;
                cl.aux3 = x.aux3;
        ...
        public static void InsertInto(string UidUserLogin,ClassTiposerviciotecnico VALUE, out string resultado)
        {
            resultado = "";
            string trx = "01601";
            CRUD(trx,UidUserLogin, VALUE, out resultado);
        }
        public static void Update(string UidUserLogin,ClassTiposerviciotecnico VALUE, out string resultado)
        {
            resultado = "";
            string trx = "01602";
            CRUD(trx,UidUserLogin, VALUE, out resultado);
        }
        public static void Delete(string UidUserLogin,ClassTiposerviciotecnico VALUE, out string resultado)
        {
            resultado = "";
            string trx = "01603";
            CRUD(trx,UidUserLogin, VALUE, out resultado);
