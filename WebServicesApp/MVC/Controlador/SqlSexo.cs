...
{
    public static class SqlSexo
    {
        private static DataTable CRUD(string trx,string UidUserLogin, ClassSexo VALUE, out string resultado, string EstadoExistencia = "A")
        {
            resultado = "";
			...
			ParamIn.Add(new ParametersProcedure(nameof(VALUE.sexo), VALUE.sexo, "".GetType().ToString(), OrentationType.In));
            ...
            #endregion
            var table = SqlProcedureExec.ExecProcedure("[dbo].[sp_sexo]", ParamIn, out ParamOut);
        ...
        public static List<ClassSexo> Select(string UidUserLogin,ClassSexo VALUE, out string resultado, string EstadoExistencia)
        {
            resultado = "";          
            string trx = "011052";
           var table =  CRUD(trx, UidUserLogin,VALUE, out resultado);
            List<ClassSexo> retorno = new List<ClassSexo>();
           var retornoAux = (from x in table.AsEnumerable()
                       select new 
                       {
					   
			uidsexo = x.Field<string>("uidsexo"),
			sexo = x.Field<string>("sexo"),
