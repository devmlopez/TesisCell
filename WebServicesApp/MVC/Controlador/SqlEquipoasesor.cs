...
{
    public static class SqlEquipoasesor
    {
        private static DataTable CRUD(string trx,string UidUserLogin, ClassEquipoasesor VALUE, out string resultado, string EstadoExistencia = "A")
        {
            resultado = "";
			...
			ParamIn.Add(new ParametersProcedure(nameof(VALUE.uidequipoasesor), VALUE.uidequipoasesor, "".GetType().ToString(), OrentationType.In));
            ...
            #endregion
            var table = SqlProcedureExec.ExecProcedure("[dbo].[sp_equipoasesor]", ParamIn, out ParamOut);
        ...
        public static List<ClassEquipoasesor> Select(string UidUserLogin,ClassEquipoasesor VALUE, out string resultado, string EstadoExistencia)
        {
            resultado = "";          
            string trx = "01620";
           var table =  CRUD(trx, UidUserLogin,VALUE, out resultado);
            List<ClassEquipoasesor> retorno = new List<ClassEquipoasesor>();
           var retornoAux = (from x in table.AsEnumerable()
                       select new 
                       {
					   
			uidequipoasesor = x.Field<string>("uidequipoasesor"),
           ...
           ClassEquipoasesor cl;
            foreach (var x in retornoAux)
            {

                cl = new ClassEquipoasesor();

				 
			  cl.uidequipoasesor = x.uidequipoasesor;
        ...
        public static void InsertInto(string UidUserLogin,ClassEquipoasesor VALUE, out string resultado)
        {
            resultado = "";
            string trx = "01621";
            CRUD(trx,UidUserLogin, VALUE, out resultado);
        }
        public static void Update(string UidUserLogin,ClassEquipoasesor VALUE, out string resultado)
        {
            resultado = "";
            string trx = "01622";
            CRUD(trx,UidUserLogin, VALUE, out resultado);
        }
        public static void Delete(string UidUserLogin,ClassEquipoasesor VALUE, out string resultado)
        {
            resultado = "";
            string trx = "01623";
            CRUD(trx,UidUserLogin, VALUE, out resultado);
